...
@Service
@Slf4j
public class ReadQrService implements IReadQrService {
    @Override
    ...
    private void handleCase38(String content, VietQR vietQR) {
        try {
    ...
    private void handleCase62(String content, VietQR vietQR) {
        try {
            int totalLength = content.length();
            while (totalLength > 0) {
                String id = content.substring(0, 2);
                ...
                String subContent = content.substring(4, 4 + subContentLength);
                if (id.equals("01")) {
                    vietQR.setBillNumber(subContent);
                } else if (id.equals("02")) {
                    vietQR.setMobilePhone(subContent);
                } else if (id.equals("03")) {
                    vietQR.setStoreLable(subContent);
                } else if (id.equals("04")) {
                    vietQR.setLoyaltyNumber(subContent);
                } else if (id.equals("05")) {
                    vietQR.setReferenceLable(subContent);
                } else if (id.equals("06")) {
                    vietQR.setCustomerLable(subContent);
                } else if (id.equals("07")) {
                    vietQR.setTeminalLable(subContent);
                } else if (id.equals("08")) {
                    vietQR.setPurpose(subContent);
                }
                totalLength -= (4 + subContentLength);
                content = content.substring(4 + subContentLength);
            }
